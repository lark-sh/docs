...
This tells Lark how to validate the RS256-signed Firebase Auth tokens your clients send.
...
3. Lark's proxy layer validates the token's RS256 signature using Firebase's public keys.
...
<Note>Firebase Auth tokens are validated by Lark's proxy layer, not the database server itself. The proxy verifies the RS256 signature against Firebase's public keys and passes the validated identity to the database.</Note>
