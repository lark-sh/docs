...
Lark's REST API is fully compatible with the Firebase Realtime Database REST API. If your backend uses `https://your-project.firebaseio.com/path.json`, change the hostname to `your-project-id.larkdb.net` and it works the same way — `GET`, `PUT`, `POST`, `PATCH`, `DELETE`, query parameters, ETags, and Server-Sent Events streaming are all supported.

See the [REST API documentation](/rest-api/overview) for full details.
