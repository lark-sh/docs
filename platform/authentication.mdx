...
Pass the token when connecting or call `signIn` after connecting:

```typescript
// Option 1: Authenticate on connect
const db = new LarkDatabase('my-project/my-database', {
  token: token
});
...
```
User logs in → Your backend → Generates JWT → Client → Lark validates → auth is set
```
...
Your project's secret key is available in the [Lark dashboard](https://admin.larkdb.net) under **Project Settings > Secret Key**.
