...
<Note>
Over WebTransport, volatile writes use UDP datagrams for truly unreliable delivery at maximum speed. Over WebSocket, they're sent as regular TCP messages but are still batched and coalesced on the server before broadcast.
</Note>
...
Mark a path as volatile by adding `".volatile": true` in your [security rules](/platform/security-rules):
...
<Warning>
If a volatile path has a rule that references `data` or `root`, the rule evaluates to `false`, and the read or write is silently denied. There's no error message; the operation just fails. Make sure your volatile path rules only use the allowed variables listed above.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Transports" icon="network-wired" href="/platform/transports">
    WebSocket vs WebTransport, and how volatile data benefits from UDP datagrams.
  </Card>
  <Card title="Limits" icon="gauge-high" href="/platform/limits">
    Size limits and rate limits, including volatile exceptions.
  </Card>
</CardGroup>
