...
Use `lark.databaseId` and `lark.projectId` to write rules that are aware of which database they're running against. This is especially useful when you have one database per game session or room — a single set of rules can enforce that users only access the database they've been assigned to.
