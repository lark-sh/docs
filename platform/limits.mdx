...
Lark enforces the same limits as Firebase Realtime Database, with one exception: there are no rate limits on [volatile](/platform/volatile) writes or events.
...
To scale beyond 200,000 connections, use multiple databases. See [database routing](/firebase/database-routing) for patterns on splitting data across databases.
...
<CardGroup cols={2}>
  ...
  <Card title="Database routing" icon="route" href="/firebase/database-routing">
    Scale beyond single-database limits by splitting data across databases.
  </Card>
</CardGroup>
