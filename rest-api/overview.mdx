...
The REST API is fully compatible with the [Firebase Realtime Database REST API](https://firebase.google.com/docs/reference/rest/database). If you have existing code that hits Firebase's `/.json` endpoints, it works with Lark by changing the hostname.
...
<Tip>If you're migrating a legacy Firebase application, you'll likely want to use the database-in-subdomain format so you don't have to change your URLs.</Tip>
